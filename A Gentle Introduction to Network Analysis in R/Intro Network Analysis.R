# R code to accompany "A Gentle Introduction to Network Analysis in R"
# Melissa Whatley, PhD

# Code based on González Canché (2019)
...
# Moving from a graph to a matrix
...
plotvar<-centrality$closeness
# Choose which color scheme to use
# display.brewer.all()
plotclr<-brewer.pal(nclr, "Greens")
# What cutoff do we use to decide color categories, here I chose to divide up the values into equal parts
class<-classIntervals(plotvar,nclr, style="equal")
# Extracts classes from classIntervals to assign colors
...
title(main="Sociogram Indicating Closeness Centrality", col.main="black")
legend("bottomleft", legend=names(attr(colcode, "table")), fill=attr(colcode, "palette"), title="Closeness Centrality")
